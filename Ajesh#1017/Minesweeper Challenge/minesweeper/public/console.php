...
elseif (isset($session_arr['grid']))
{
	$grid = $session_arr['grid'];
}
...
$session_arr['grid'] = $grid;
//$_SESSION['grid'] = $grid;
//var_dump($_SESSION['grid']);exit();
...
if ($row_col)
{
	...
	if ( ! $grid->isGameOver())
	{
		...
		$reveal_position = explode(' ', $row_col); 
...
foreach ($grid->getGrid() as $row_key => $row)
{
	...
	$table->addRow();
	$table->addColumn($rw_no);$rw_no++;
foreach ($row as $column_key => $square)
{
	if($grid->isGameOver()) {
		if($square->numberOfSurroundingGameOverSquares() > 0 and !$square->isGameOver()) {
			$table->addColumn($square->numberOfSurroundingGameOverSquares());
		}
		else if($square->isGameOver() and ! $square->isRevealed()) {
			$table->addColumn('*');
		}
		else if($square->numberOfSurroundingGameOverSquares() == 0 and !$square->isGameOver()){
			$table->addColumn('--');
		}
		else if ($square->isRevealed() AND $square->isGameOver()) {
			$table->addColumn('(*)');
		}

	}
	else if($grid->isWonByPlayer()) {
		if($square->numberOfSurroundingGameOverSquares() > 0 and !$square->isGameOver()) {
			$table->addColumn($square->numberOfSurroundingGameOverSquares());
		}
		else if($square->isGameOver() and ! $square->isRevealed()) {
		...
		if($square->numberOfSurroundingGameOverSquares() > 0 AND $square->isRevealed()) {
			$table->addColumn($square->numberOfSurroundingGameOverSquares());
		}
		else if($square->isRevealed() OR $grid->isGameOver()) {
...
if($grid->isGameOver()) {
	...
	echo 'CONGRATZZZZ... YOU WON THE GAME !!!!'; break;
}
$row_col = readline('Enter row <space> column :');
}
/**
 *  Render
 */
